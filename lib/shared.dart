...
PxArtwork pxArtwork(Map<String,dynamic> dat, {int rank = 0}) {
  return PxArtwork(data: dat, rank: rank);
}
/// as used in [AuthorInfo]
class AuthorInfoArtwork extends StatefulWidget {
  final Map<String,dynamic> data;
  const AuthorInfoArtwork({super.key, required this.data});

  @override
  State<AuthorInfoArtwork> createState() => _AuthorInfoArtworkState();
}

class _AuthorInfoArtworkState extends State<AuthorInfoArtwork> {
  bool bookmarked = false;
  @override
  Widget build(context) {
    var id = widget.data["illustId"]??widget.data["id"];
      ...
      children: [
        GestureDetector(
          ...
          child:Container(
            ...
            clipBehavior: Clip.hardEdge,
            child: pxImage(widget.data["url"],width:85,height:85)
          ),
        ),
        Positioned(
          bottom: 0,
...
class PxSimpleArtwork extends StatefulWidget {
  ...
  const PxSimpleArtwork({super.key, required this.data, this.isCurrent = false});
        ...
        GestureDetector(
          ...
          child:Container(
            ...
            foregroundDecoration: BoxDecoration(
              borderRadius: BorderRadius.circular(4),
              color: Colors.black.withOpacity(widget.isCurrent?0.5:0)
            ),
            clipBehavior: Clip.hardEdge,
            child: pxImage(widget.data["url"],width:120,height:120)
          ),
        ),
        Positioned(
          bottom: 0,
            ...
            return Center(child:Column(
              children: [
                Container(
                  ...
                  child:dd["background"]!=null?pxImage(dd["background"]["url"]):const SizedBox(height: 1, width: 1,)
                ),
                const SizedBox(height: 12,),
                CircleAvatar(backgroundImage: pxImageUncached(dd["imageBig"],width: 45, height: 45).image),
                ...
                Row(
                  ...
                  children: [
                    if (dd["webpage"]!=null) Padding(
                      ...
                      child: IconButton(onPressed: ()=>launchUrl(dd["webpage"]), icon: SvgPicture.string(svgs["webpage"]!,width: 24,height: 24,)),
                    ),
                    ...List.from(dd["social"].entries.map((me)=>Padding(
                      padding: const EdgeInsets.only(left:2.0,right:2.0),
                      child: IconButton(onPressed:()=>launchUrl(me.value["url"]), icon: SvgPicture.string(svgs[me.key]!,width: 24,height: 24,)),
                    ))),
                    if (dd["canSendMessage"]) Padding(
                      padding: const EdgeInsets.only(left:2.0,right:2.0),
                      child: SvgPicture.string(svgs["message"]!,width: 24,height: 24,),
                    )
                  ],
                ),
                const SizedBox(height: 12,),
                FilledButton(
                  onPressed: ()=>navigate("/user/"+dd["userId"]), 
